...
const createStock = async (stock) => {
    try {
        const createdStock = await db.one("INSERT INTO stocks (name,price,quantity,purchaseDate, optionType, expiryDate) VALUES ($1,$2,$3,$4,$5,$6) RETURNING *", [stock.name, stock.price, stock.quantity, stock.purchaseDate, stock.optionType, stock.expiryDate])
        return createdStock
    } catch (err) {
        return err
    }
